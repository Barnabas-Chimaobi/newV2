...
export default function ManageForm() {
    ...
    const TableObjprosession = { programmeName: "", programmeId: "", sessionId: "", sessionName: "", stepOne: "", stepThree: "", stepTwo: "", active: "", Id: "" };
    const headers = [
        {
            field: "programmeName",
            header: "Programme",
    ...
    const LoadData = async () => {
        ...
        const tableRow = data?.data?.allSetUpDone?.map((item) => {
            return {
                programmeName: item?.programmeName, programmeId: item?.programmeId, sessionId: item?.sessionId, sessionName: item?.sessionName, stepOne: item?.stepOne,
                stepThree: item?.stepThree, stepTwo: item?.stepTwo, active: item?.active, Id: item?.id
            };
        });
        setavailableForms(tableRow)
        console.log(data?.data?.allSetUpDone, "data from useeffect")
        const sessionDataFromB = await session();
        setSession(sessionDataFromB?.data?.allSession?.map((item) => { return { Id: item?.id, Name: item?.name } }));
        const programmeD = await programmeL();
        setprogramme(programmeD?.data?.allProgramme?.map((item) => { return { Id: item?.id, Name: item?.name } }))
