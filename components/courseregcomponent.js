...
export default function Courseregcomponent() {
	...
	const [
		saveCourseReg,
		{
			loading: saveCourseRegLoad,
			error: saveCourseRegError,
			data: saveCourseRegData,
		},
	] = useMutation(SAVE_STUDENT_COURSE_REGISTER);
	...
	const SaveRegisterCourses = async () => {
		...
		const Save = await saveCourseReg({
			...
			router.push(
				Constant.BASE_URL +
					`/common/courseregistration/` +
					ex?.data?.saveStudentCourseReg?.id
			);
		});
	};
							...
							(x, index) => (
								...
								(
									<AccordionTab
										...
										<Accordion
											activeIndex={x?.activeIndex == null ? 0 : x?.activeIndex}>
											{x?.semesterCourseDisplayDtos?.map((y, indexy) => (
												<AccordionTab
													...
													<p className="m-0">{/* First Semester content */}</p>
													<DataTable
														value={y?.results}
														...
														dataKey="courseId"
														emptyMessage="No course(s) available yet.">
														<Column
														...
														<Column
															field="creditUnit"
															header="Credit Unit"></Column>
														<Column
															field="courseType"
															header="Course Type"></Column>
	...
	return (
		<>
			...
			<Card title="">
				<Dialog
					header="Course Registration Preview"
					visible={visible}
					style={{ width: "50vw" }}
