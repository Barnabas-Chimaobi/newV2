...
export default function GenericForm({
	...
	const dropdowns = async () => {
		...
		if (data !== null) {
			const stateList = data?.mainPages?.flatMap((item, index) =>
	...
	const handleTabNavigation = (targetTabIndex) => {
		console.log(targetTabIndex, "index=====");
		// saveAndContinue();
		// console.log(data?.applicantForm?.mainPages.length, "index lemgth")
		if (targetTabIndex >= 0 && targetTabIndex < data?.mainPages?.length) {
									...
									<div className="card bg-white">
										...
										<div className="card-body">
											<ul className="nav nav-tabs nav-tabs-solid nav-tabs-rounded">
												{data?.mainPages?.map((item0, index0) => (
													<>
